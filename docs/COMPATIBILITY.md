...
| SceneGraph via Run                              | partial      | Canvas runtime with emitted live-node inspection   |
| Remote keys via Run                             | partial      | D-pad, OK, Back, rewind, play, fast-forward        |
| Runtime hot reload                              | experimental | Serialized full restart; runtime state is reset    |
| Live runtime Inspector                          | experimental | Nodes, fields, focus, calls, and observer matches  |
| Runtime call source                             | experimental | Emitted call-site location; full stack unavailable |
| IEDB reference navigation                       | experimental | Automated visual route/detail/Back flow            |
